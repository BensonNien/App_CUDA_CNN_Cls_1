...
// Utility
float CPU_Algo_Lib::EvlElapsedTime()
{
	return clock() / CLOCKS_PER_SEC;
}

void CPU_Algo_Lib::RandomMatrix(size_t size_row, size_t size_col, float* p_kernel)
{
...
void CPU_Algo_Lib::ConvNValid(float* p_matrix, float* p_kernel, size_t map_size_row, size_t map_size_col, size_t kernel_size_row, size_t kernel_size_col, float* p_outmatrix)
{
...
void CPU_Algo_Lib::ActiveRelu(float* p_matrix, float bias, size_t m, size_t n)
{
...
void CPU_Algo_Lib::CalExpone(float* p_matrix, float bias, size_t m, size_t n)
{
	for (size_t i = 0; i < m; i++)
	{
		for (size_t j = 0; j < n; j++)
...
void CPU_Algo_Lib::CalConvArrayPlus(float* p_x, float* p_y, size_t m, size_t n)
{
...
void CPU_Algo_Lib::CalFCHArrayPlus(float* p_x, float* p_y, size_t m, size_t n)
{
...
void CPU_Algo_Lib::CalSampArrayPlus(float* p_x, float* p_y, size_t m, size_t n)
{
...
void CPU_Algo_Lib::CalArrayPlus(float* p_x, float* p_y, size_t m, size_t n)
{
	for (size_t i = 0; i < m; i++)
	{
		for (size_t j = 0; j < n; j++)
...
void CPU_Algo_Lib::ScaleMatrix(float* p_matrix, CPU_Algo_Lib::RectSize scale, size_t matrix_rows, size_t matrix_cols, float* p_out_matrix)
{
...
void CPU_Algo_Lib::Rot180(float* p_matrix, size_t m, size_t n, float* p_rot_matrix)
{
...
void CPU_Algo_Lib::ConvNSampFull(float* p_matrix, float* p_kernel, size_t m, size_t n, size_t km, size_t kn, float* p_out_matrix, float* p_extend_matrix)
{
	...
	CPU_Algo_Lib::ConvNValid(p_extend_matrix, p_kernel, (m + 2 * (km - 1)), (n + 2 * (kn - 1)), km, kn, p_out_matrix);

}

void CPU_Algo_Lib::MatrixDrelu(float** p_matrix, size_t m, size_t n, float** p_M)
{
	for (size_t i = 0; i < m; i++)
	{
		for (size_t j = 0; j < n; j++)
...
void CPU_Algo_Lib::MatrixDreluFChidden(float* p_matrix, size_t m, size_t n, float* p_M)
{
	for (size_t i = 0; i < m; i++)
	{
		for (size_t j = 0; j < n; j++)
...
void CPU_Algo_Lib::MatrixDreluConv(float* p_matrix, size_t m, size_t n, float* p_M)
{
	for (size_t i = 0; i < m; i++)
	{
		for (size_t j = 0; j < n; j++)
...
void CPU_Algo_Lib::MatrixDsigmoid(float** p_matrix, size_t m, size_t n, float** p_M)
{
	for (size_t i = 0; i < m; i++)
	{
		for (size_t j = 0; j < n; j++)
...
void CPU_Algo_Lib::MatrixDsigmoidFChidden(float** p_matrix, size_t m, size_t n, float* p_M)
{
	for (size_t i = 0; i < m; i++)
	{
		for (size_t j = 0; j < n; j++)
...
void CPU_Algo_Lib::Kronecker(float** p_matrix, CPU_Algo_Lib::RectSize scale, size_t m, size_t n, float** p_outmatrix)
{
	for (size_t i = 0; i < m; i++) {
		for (size_t j = 0; j < n; j++) {
...
void CPU_Algo_Lib::CalKronecker(float* p_nextlayer_matrix, CPU_Algo_Lib::RectSize scale, size_t nextlayer_matrix_rows, size_t nextlayer_matrix_cols, float* p_out_matrix, size_t layer_out_matrix_rows, size_t layer_out_matrix_cols)
{
...
void CPU_Algo_Lib::MatrixMultiply(float** p_matrix1, float** p_matrix2, size_t m, size_t n)
{
	for (size_t i = 0; i < m; i++)
	{
		for (size_t j = 0; j < n; j++)
...
void CPU_Algo_Lib::CalMatrixMultiply(float* p_matrix1, float* p_matrix2, size_t m, size_t n)
{
	for (size_t i = 0; i < m; i++)
	{
		for (size_t j = 0; j < n; j++)
...
void CPU_Algo_Lib::CalErrorsSum(float* p_errors, size_t idx_outmap, size_t outmap_num, size_t outmap_rows, size_t outmap_cols, size_t batch_size, float* p_m)
{
	float sum = 0.0;
...
float CPU_Algo_Lib::CalErrorSum(float* p_error, size_t m, size_t n)
{
	float sum = 0.0;
	for (size_t i = 0; i < m; i++) {
		for (size_t j = 0; j < n; j++) {
...
void CPU_Algo_Lib::CalArrayDivide(float* p_M, size_t batch_size, size_t m, size_t n)
{
	for (size_t i = 0; i < m; i++)
	{
		for (size_t j = 0; j < n; j++)
...
void CPU_Algo_Lib::CalArrayMultiply(float* p_matrix, float val, size_t m, size_t n)
{
...
size_t CPU_Algo_Lib::FindIndex(float* p_batch_maps, size_t map_num, size_t map_rows, size_t map_cols)
{
...
size_t CPU_Algo_Lib::FindIndex(float* p_batch_labels, size_t map_num)
{
	size_t index = 0;
...
void CPU_Algo_Lib::SetInLayerValue(float* p_maps, float** p_sum, size_t m, size_t n)
{
	for (size_t i = 0; i < m; i++)
	{
		for (size_t j = 0; j < n; j++)
...
void CPU_Algo_Lib::SetKernelValue(float* p_maps, float* p_sum, size_t m, size_t n)
{
	for (size_t i = 0; i < m; i++)
	{
		for (size_t j = 0; j < n; j++)
...
CPU_Algo_Lib::CPUCNNLayer CPU_Algo_Lib::CPUCNNLayer::CreateInputLayer(size_t input_map_num, CPU_Algo_Lib::RectSize map_size)
{
...
CPU_Algo_Lib::CPUCNNLayer CPU_Algo_Lib::CPUCNNLayer::CreateConvLayer(size_t input_map_num, size_t output_map_num, CPU_Algo_Lib::RectSize kernel_size)
{
	CPU_Algo_Lib::CPUCNNLayer layer;
	layer.layer_type_ = 'C';
	layer.in_map_num_ = input_map_num;
...
CPU_Algo_Lib::CPUCNNLayer CPU_Algo_Lib::CPUCNNLayer::CreateSampLayer(CPU_Algo_Lib::RectSize scale_size)
{
	CPU_Algo_Lib::CPUCNNLayer layer;
	layer.layer_type_ = 'S';
...
CPU_Algo_Lib::CPUCNNLayer CPU_Algo_Lib::CPUCNNLayer::CreateFullyConnectedHiddenLayer(size_t input_element_num, size_t output_element_num, size_t class_num)
{
	CPU_Algo_Lib::CPUCNNLayer layer;
	...
	layer.layer_type_ = 'H';
	layer.map_size_ = CPU_Algo_Lib::RectSize(1, 1);
	layer.out_map_num_ = output_element_num;
	return layer;

}
CPU_Algo_Lib::CPUCNNLayer CPU_Algo_Lib::CPUCNNLayer::CreateOutputLayer(size_t class_num)
{
	CPU_Algo_Lib::CPUCNNLayer layer;
	layer.class_num_ = class_num;
	layer.layer_type_ = 'O';
...
void CPU_Algo_Lib::CPUCNNLayer::InitKernel(size_t front_map_num) {
...
//for adding momentum
void CPU_Algo_Lib::CPUCNNLayer::InitLastStepDeltaKernel(size_t front_map_num)
{
...
void CPU_Algo_Lib::CPUCNNLayer::InitOutputKernel(size_t front_map_num, CPU_Algo_Lib::RectSize Kernel_size)
{
...
//for adding momentum
void CPU_Algo_Lib::CPUCNNLayer::InitOutputLastStepDeltaKernel(size_t front_map_num, CPU_Algo_Lib::RectSize Kernel_size)
{
	kernel_size_ = Kernel_size;
	vec_laststep_delta_kernel_.reserve(front_map_num * out_map_num_ * kernel_size_.rows_ * kernel_size_.cols_);
	vec_laststep_delta_kernel_.resize(front_map_num * out_map_num_ * kernel_size_.rows_ * kernel_size_.cols_);
...
void CPU_Algo_Lib::CPUCNNLayer::InitErros(size_t batch_size)
{
...
void CPU_Algo_Lib::CPUCNNLayer::InitOutputMaps(size_t batch_size)
{
...
void CPU_Algo_Lib::CPUCNNLayer::InitBias(size_t front_map_num, size_t idx_iter)
{
	vec_bias_.reserve(out_map_num_);
	vec_bias_.resize(out_map_num_);
	vec_bias_.assign(vec_bias_.size(), 0.1);
}
void CPU_Algo_Lib::CPUCNNLayer::SetError(size_t num_batch, size_t map_no, size_t map_x, size_t map_y, float error_val)
{
...
void CPU_Algo_Lib::CPUCNNLayer::SetFCHLayerError(size_t num_batch, size_t map_no, float* p_matrix, size_t m, size_t n)
{
	size_t shift_idx_error_batch_map = num_batch * out_map_num_ * map_size_.rows_ * map_size_.cols_;
	size_t shift_idx_error_out_map = map_no * map_size_.rows_ * map_size_.cols_;
	for (size_t i = 0; i < m; i++)
...
void CPU_Algo_Lib::CPUCNNLayer::SetSampLayerError(size_t num_batch, size_t map_no, float* p_matrix, size_t m, size_t n)
{
	size_t shift_idx_error_batch_map = num_batch * out_map_num_ * map_size_.rows_ * map_size_.cols_;
	size_t shift_idx_error_out_map = map_no * map_size_.rows_ * map_size_.cols_;
	for (size_t i = 0; i < m; i++)
...
void CPU_Algo_Lib::CPUCNNLayer::SetConvLayerError(size_t num_batch, size_t map_no, float* p_matrix, size_t m, size_t n)
{
	size_t shift_idx_error_batch_map = num_batch * out_map_num_ * map_size_.rows_ * map_size_.cols_;
	size_t shift_idx_error_out_map = map_no * map_size_.rows_ * map_size_.cols_;
	for (size_t i = 0; i < m; i++)
