...
namespace CPU_Algo_Lib
{
	...
	void ScaleMatrix(float* p_matrix, CPU_Algo_Lib::RectSize scale, size_t matrix_rows, size_t matrix_cols, float* p_out_matrix);//sampling
	...
	void Kronecker(float** p_matrix, CPU_Algo_Lib::RectSize scale, size_t m, size_t n, float** p_outmatrix);
	void CalKronecker(float* p_nextlayer_matrix, CPU_Algo_Lib::RectSize scale, size_t nextlayer_matrix_rows, size_t nextlayer_matrix_cols, float* p_out_matrix, size_t layer_out_matrix_rows, size_t layer_out_matrix_cols);
	...
	private:
		...
		size_t out_map_num_;
		char layer_type_;
		CPU_Algo_Lib::RectSize map_size_;
		CPU_Algo_Lib::RectSize scale_size_;
		CPU_Algo_Lib::RectSize kernel_size_;
	...
	public:
		...
		CPUCNNLayer CreateInputLayer(size_t input_map_num, CPU_Algo_Lib::RectSize map_size);
		CPUCNNLayer CreateConvLayer(size_t input_map_num, size_t output_map_num, CPU_Algo_Lib::RectSize kernel_size);
		CPUCNNLayer CreateSampLayer(CPU_Algo_Lib::RectSize scale_size);
		CPUCNNLayer CreateFullyConnectedHiddenLayer(size_t input_element_num, size_t output_element_num, size_t class_num);
		//CPUCNNLayer CreateOutputLayer(size_t input_element_num, size_t output_element_num, size_t class_num);
		CPUCNNLayer CreateOutputLayer(size_t class_num);

		void InitKernel(size_t front_map_num);
		void InitLastStepDeltaKernel(size_t front_map_num);//for adding momentum
		void InitOutputKernel(size_t front_map_num, CPU_Algo_Lib::RectSize Kernel_size);
		void InitOutputLastStepDeltaKernel(size_t front_map_num, CPU_Algo_Lib::RectSize Kernel_size);//for adding momentum
		...
		CPU_Algo_Lib::RectSize GetMapSize() {
			return map_size_;
		}
		void SetMapSize(CPU_Algo_Lib::RectSize map_size) {
			this->map_size_ = map_size;
		}
		void SetOutMapNum(size_t out_map_num) {
			this->out_map_num_ = out_map_num;
		}
		CPU_Algo_Lib::RectSize GetKernelSize() {
			return kernel_size_;
		}
		CPU_Algo_Lib::RectSize GetScaleSize() {
			return scale_size_;
		}
